...
class OsloScrapper(OsloAlignment):
    ...
    def get_annotations(self,volumes,opf_path):
        annotations={}
        for volume in volumes:
            span = self.get_spans(opf_path,volume)
            annotation =  {uuid4().hex:{"span":span}}
            annotations.update(annotation)

        return annotations

    def get_spans(self,opf_path,volume):
    ...
    def create_meta_index(self,pecha,volumes,chapter_no_title,base_id_title_maps):
        opf_path = f"{self.root_opf_path}/{pecha['pecha_id']}/{pecha['pecha_id']}.opf"
        opf = OpenPechaFS(path=opf_path)
        annotations = self.get_annotations(volumes,opf_path)
    ...
    def scrap(self,url):
        ...
        for pecha in self.pechas:
            volumes = self.get_volumes(pecha)
            self.create_meta_index(pecha,volumes,chapter_to_title,base_id_title_maps)
    ...
    def scrap_all(self):
        ...
        for val in self.get_page():
            self.scrap("https://www2.hf.uio.no/polyglotta/index.php?page=volume&vid=560")
            """ try:
                if "http" in val['ref']:
                    print(val["ref"])
                    self.scrap(val['ref'])
    ...
    def create_tmx_zip(self,tmx_path):
        self._mkdir(Path(self.root_tmx_zip_path))
        zip_path = f"{self.root_tmx_zip_path}/{self.pecha_name}.zip"
